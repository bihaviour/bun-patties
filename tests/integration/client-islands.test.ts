...
function matchesSel(el: { tagName: string; attrs: Record<string, string> }, sel: string): boolean {
  ...
  if (!m) return false
  return el.tagName === "SCRIPT" && "data-props" in el.attrs && el.attrs["data-for"] === m[1]
}
...
test("createClient.register + hydrateAll calls hydrateRoot per marker with parsed props", async () => {
  ...
  const { createClient } = await import(`../../src/client/index.ts?t=${Date.now()}`)
  const client = createClient()
  const Counter = (props: any) => React.createElement("button", null, "n=" + props.start)
  client.register("counter", Counter)
  client.hydrateAll()

  expect(hydrateRoot).toHaveBeenCalledTimes(1)
  const call = hydrateRoot.mock.calls[0] as unknown as [any, React.ReactElement]
...
test("multi-island build emits a single shared React runtime chunk", async () => {
  ...
  const chunks: string[] = []
  for await (const f of glob.scan({ cwd: outDir + "/client", onlyFiles: true })) chunks.push(f)
  ...
  for (const c of chunks) {
    ...
    if (src.includes("react.dev") || src.includes("ReactDOMHydrationRoot")) runtimeChunks++
  }
...
test("generated client-entry imports patties/client and registers each island", async () => {
  ...
  const generated = await Bun.file(appDir + "/patties-gen/client-entry.ts").text()
