...
test("zero-island project produces empty client manifest and no client files", async () => {
  ...
  const found: string[] = []
  try {
    for await (const f of glob.scan({ cwd: join(outDir, "client"), onlyFiles: true })) {
      found.push(f)
    }
  } catch {
...
test("one-island project emits an entry chunk and registers the island", async () => {
  ...
  expect(res.clientManifest.islands.counter!.startsWith("/_patties/client/")).toBeTrue()

  // Spec acceptance: "exactly one chunk" for a single-island project.
  const glob = new Bun.Glob("**/*.js")
  let count = 0
  for await (const _ of glob.scan({ cwd: join(outDir, "client"), onlyFiles: true })) count++
  expect(count).toBe(1)
})
...
test("server bundle contains inlined route table and no runtime filesystem scan", async () => {
  ...
  expect(source).toMatch(/bunPattern:\s*"\/api\/ping"|"bunPattern":\s*"\/api\/ping"/)
...
test("public assets are copied with content preserved", async () => {
  ...
  const original = await Bun.file(join(FIXTURES, "build-app/app/public/robots.txt")).text()
...
test("edge target emits a `export default { fetch }` worker module", async () => {
  ...
  const source = await Bun.file(res.serverEntry).text()
  expect(source).toMatch(/export\s*\{[^}]*\s+as\s+default\s*\}|export\s+default\s*\{/)
