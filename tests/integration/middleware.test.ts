...
async function boot(appDir: string): Promise<LiveServer> {
  ...
  return startServer({ routes, fallback, port: 0, hostname: "127.0.0.1" }) as unknown as LiveServer
}
...
describe("spec 07 — middleware via real Bun.serve", () => {
  describe("basic-app: logging + 404 pass-through", () => {
    ...
    beforeAll(async () => {
      counter = (await import(join(FIXTURES, "basic-app/app/middleware.ts"))) as typeof counter
