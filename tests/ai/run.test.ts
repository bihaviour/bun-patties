...
import { z } from "zod"
import { registerAgent, registerTool, getAgent, __resetRegistry } from "../../src/ai/registry.ts"
...
describe("agent run", () => {
  ...
  test("tool with bad input throws ToolInputInvalid", async () => {
    ...
    const ctx = createAiContext({
      anthropic: fakeAnthropic([
        events([
          { type: "content_block_start", content_block: { type: "tool_use", id: "tu1", name: "search" } },
          { type: "content_block_delta", delta: { type: "input_json_delta", partial_json: '{"city":' } },
          { type: "content_block_delta", delta: { type: "input_json_delta", partial_json: "123}" } },
          { type: "content_block_stop" },
          { type: "message_stop" },
        ]),
      ]),
  ...
  test("tool happy path: handler runs and second round closes", async () => {
    ...
    const ctx = createAiContext({
      anthropic: fakeAnthropic([
        events([
          { type: "content_block_start", content_block: { type: "tool_use", id: "tu1", name: "echo" } },
          { type: "content_block_delta", delta: { type: "input_json_delta", partial_json: '{"s":"hi"}' } },
          { type: "content_block_stop" },
          { type: "message_stop" },
        ]),
        events([
