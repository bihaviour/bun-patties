...
import { registerAgent, __resetRegistry } from "../../src/ai/registry.ts"
...
describe("agent triggers", () => {
  ...
  test("skips and warns when a FS route claims the path", () => {
    ...
    const routes: BunRoutes = {}
    const entries: RouteEntry[] = [
      { filePath: "/x/api/y/chat.ts", bunPattern: "/api/y/chat", kind: "api", segments: [] },
    ]
    const warns: string[] = []
  ...
  test("invalid trigger format warns + skips", () => {
    registerAgent({ name: "z", model: "m", triggers: ["totally-bad"] as never })
    const routes: BunRoutes = {}
    const warns: string[] = []
    const orig = console.warn
    console.warn = (...args: unknown[]) => warns.push(args.map(String).join(" "))
