/* =============================================================================
   Patties — site behaviour (single source of truth).
   Injects the top site-header nav (shared across home, docs, specs, release,
   changelog), wires the theme toggle, and — when a page declares window.SECTION
   — builds the left sidebar, "on this page" TOC, prev/next, heading anchors,
   code-copy buttons, and scrollspy. Works over HTTP (no build step).
   Adapted from the Twill DB site behaviour.
   ============================================================================= */
(function () {
  "use strict";
  ...
  var REPO = "https://github.com/bihaviour/bun-patties-framework";

  // ---- Analytics config -----------------------------------------------------
  // GA4 Measurement ID. Set the real "G-XXXXXXXXXX" id from the GA4 property
  // (Admin → Data Streams) to enable privacy-first, consent-gated analytics.
  // While empty (or not starting with "G-"), analytics stays inert: no Google
  // script loads and no consent banner is shown.
  var GA4_MEASUREMENT_ID = "";

  // ---- Top site header (Home / Docs / Release / Changelog) ------------------
  var NAV = [
  ...
  function buildHeader() {
    ...
    header.innerHTML =
      '<a class="site-brand" href="' + BASE + 'index.html" aria-label="Patties — home">'
        + '<span class="brand-mark" aria-hidden="true">'
          + '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">'
          + '<path d="M3 14.5c0-1 .8-1.8 1.8-1.8h14.4c1 0 1.8.8 1.8 1.8 0 2.3-1.9 4.2-4.2 4.2H7.2A4.2 4.2 0 0 1 3 14.5Z" fill="currentColor"/>'
          + '<path d="M5 11.2C5 7.4 8.1 4.5 12 4.5s7 2.9 7 6.7c0 .5-.4.8-.9.8H5.9c-.5 0-.9-.3-.9-.8Z" fill="currentColor" opacity=".55"/>'
          + '<circle cx="9" cy="8.4" r="1" fill="var(--bg)"/><circle cx="13.2" cy="7.3" r="1" fill="var(--bg)"/><circle cx="15.4" cy="9.4" r="1" fill="var(--bg)"/>'
          + '<rect x="4.4" y="20" width="15.2" height="1.8" rx=".9" fill="currentColor" opacity=".35"/>'
          + '</svg></span>'
        + '<span class="brand-word">patties</span></a>'
