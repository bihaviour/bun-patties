...
test.skipIf(!hasGit())(
	...
	async () => {
		...
		await Bun.$`git -c user.email=t@t -c user.name=t commit --allow-empty -m base`
			...
			.quiet();
		const head = (
			await Bun.$`git rev-parse HEAD`.cwd(outer).env(cleanEnv).text()
		).trim();
		...
		expect(
			(await Bun.$`git rev-parse HEAD`.cwd(outer).env(cleanEnv).text()).trim(),
		).toBe(head);
		const log = await Bun.$`git log --oneline`.cwd(outer).env(cleanEnv).text();
