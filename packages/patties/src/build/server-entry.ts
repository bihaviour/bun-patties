...
export function generateServerEntry(input: ServerEntryInput): string {
	...
	const routeImports = importOrder
		.map(
			({ e, idx }) => `import * as R${idx} from ${JSON.stringify(e.filePath)}`,
		)
		.join("\n");
	...
	const agentImports = agents
		.map((a, i) => `import AGENT_MOD_${i} from ${JSON.stringify(a.filePath)}`)
		.join("\n");
	const toolImports = tools
		.map((t, i) => `import TOOL_MOD_${i} from ${JSON.stringify(t.filePath)}`)
		.join("\n");
	const jobImports = jobs
		.map((j, i) => `import JOB_MOD_${i} from ${JSON.stringify(j.filePath)}`)
		.join("\n");

	const aiImports =
		agents.length || tools.length || jobs.length
