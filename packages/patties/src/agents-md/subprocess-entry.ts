...
async function handle(
	req: Request,
): Promise<{ id: string; ok: boolean; data?: unknown; error?: string }> {
	try {
		const mod = (await import(req.file)) as { default?: unknown };
		const cfg = mod.default as Record<string, unknown> | undefined;
		if (!cfg)
			return { id: req.id, ok: false, error: "module has no default export" };
		if (req.kind === "tool") {
...
main().catch((err) => {
	process.stderr.write(
		`subprocess-entry fatal: ${(err as Error)?.message ?? String(err)}\n`,
	);
	process.exit(1);
});
