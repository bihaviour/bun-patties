...
async function resolveServiceName(cwd: string, override?: string): Promise<string> {
...
export async function loadSecrets(
  ...
  try {
    ...
    for (const key of config.secrets) {
      ...
      try {
        ...
        if (/libsecret|keyring|keychain|backend/i.test(msg)) {
          console.warn(`[patties] Bun.secrets backend unavailable; falling back to env (${msg})`)
          backendOk = false
        }
    ...
    console.warn(`[patties] loadSecrets skipped: ${(err as Error)?.message ?? String(err)}`)
  }
}
