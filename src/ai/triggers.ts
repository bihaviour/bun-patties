...
export function registerAgentTriggers(
  ...
  for (const e of entries) {
    ...
    if (e.kind === "api" && mod) {
      for (const m of ["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS", "HEAD"] as HTTPMethod[]) {
  ...
  for (const a of listAgents()) {
    for (const trig of a.config.triggers ?? []) {
      ...
      if (!parsed) {
        console.warn(`[patties] agent "${a.config.name}": invalid trigger "${trig}" (expected "METHOD /path").`)
        continue
      }
  ...
  return async (req: Request & { params?: Record<string, string> }): Promise<Response> => {
    ...
    try {
      ...
      if (e.name === "ToolInputInvalid") {
        return new Response(JSON.stringify({ error: "ToolInputInvalid", issues: e.issues }), {
          status: 400,
...
function attachTrigger(
  ...
  if (existing && typeof existing === "object" && !(existing instanceof Response)) {
    routes[pattern] = { ...(existing as object), ...wrapped } as typeof routes[string]
  } else {
