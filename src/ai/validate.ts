import type { AgentConfig, ToolConfig, JobConfig } from "./types.ts"
...
export function validateNamedModules(
  ...
  if (problems.length > 0) {
    throw new Error(`patties: ${kind} validation failed\n  ` + problems.join("\n  "))
  }
}
...
export function validateAgentToolReferences(
  ...
  if (problems.length > 0) {
    throw new Error("patties: agent tool reference validation failed\n  " + problems.join("\n  "))
  }
}
...
export function validateJobs(jobs: NamedModule<JobConfig>[]): void {
  ...
  for (const j of jobs) {
    if (!j.config.schedule || typeof j.config.schedule !== "string") {
      problems.push(`${j.filePath}: job "${j.config.name}" missing "schedule".`)
    }
  ...
  if (problems.length > 0) {
    throw new Error("patties: job validation failed\n  " + problems.join("\n  "))
  }
}
