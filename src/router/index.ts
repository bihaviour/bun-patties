...
import { compose, makeContext, type Middleware } from "../middleware/index.ts"
...
export interface Plugin {
  ...
  setup?: (api: { route: (pattern: string, handlers: Partial<Record<HTTPMethod, Handler>>) => void }) => void | Promise<void>
}
...
export async function createRouter(options: RouterOptions): Promise<CompiledRouter> {
  ...
  const userMiddleware = options.middleware ?? (await loadAppMiddleware(appDir))
...
export async function createCompiledRouter(options: CompiledRouterOptions): Promise<CompiledRouter> {
  ...
  for (const entry of entries) {
    if (entry.kind === "page") {
      const handler: Handler = (req, ctx) => renderer.renderPage(entry, req, ctx)
      attach(routes, entry.bunPattern, { GET: wrap(handler, middlewareChain) })
    } else {
      const mod = (preloadedModules?.[entry.filePath] ?? (await import(entry.filePath))) as Record<string, unknown>
  ...
  for (const p of plugins) {
    ...
    await p.setup({
      route(pattern, handlers) {
        ...
        for (const [m, h] of Object.entries(handlers) as [HTTPMethod, Handler][]) {
...
function attach(routes: BunRoutes, pattern: string, methodMap: Partial<Record<HTTPMethod, Handler>>): void {
  const existing = routes[pattern]
  const bunWrapped: Record<string, (req: Request & { params: Record<string, string> }) => Promise<Response>> = {}
  for (const [m, h] of Object.entries(methodMap)) {
    if (!h) continue
    bunWrapped[m] = (req) => {
      const params = req.params ?? {}
      const ctx = makeContext(req, params)
      return Promise.resolve(h(req, ctx)).then((res) => appendCookieHeaders(res, ctx.cookies))
    }
  }
  if (existing && typeof existing === "object" && !(existing instanceof Response)) {
    routes[pattern] = { ...(existing as object), ...bunWrapped } as typeof routes[string]
  } else {
...
async function loadAppMiddleware(appDir: string): Promise<Middleware | undefined> {
