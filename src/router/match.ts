...
export function makeMatcher(pattern: string): (path: string) => Record<string, string> | null {
...
export function compilePatterns(routes: BunRoutes): Compiled[] {
  return Object.keys(routes).map((pattern) => ({ pattern, match: makeMatcher(pattern) }))
}
...
export async function dispatch(
  ...
  for (const c of list) {
    ...
    const augmented = Object.assign(req, { params, env: extras.env, execCtx: extras.execCtx })
    if (typeof value === "function") {
      return await (value as (r: Request & { params: Record<string, string> }) => Response | Promise<Response>)(augmented)
    }
    const methodHandler = (value as Record<string, unknown>)[req.method]
    if (typeof methodHandler === "function") {
      return await (methodHandler as (r: Request & { params: Record<string, string> }) => Response | Promise<Response>)(
        augmented,
      )
    }
