...
import { copyAssets, type BuiltAsset } from "./assets.ts"
...
export async function build(options: BuildOptions): Promise<BuildResult> {
  ...
  if (islands.length > 0) {
    ...
    await Bun.write(clientEntryPath, generateClientEntry(islands, { frameworkRoot }))
    ...
    if (!clientResult.success) {
      const msgs = clientResult.logs.map((l) => l.message ?? String(l)).join("\n")
      throw new Error("patties build: client bundle failed\n" + msgs)
    }

    populateClientManifest(manifest, clientResult.outputs, clientEntryPath, islands, clientOutDir)
  } else {
    // Zero-island project: write nothing to client/, leave manifest empty.
    await Bun.write(genDir + "/client-entry.ts", generateClientEntry([], { frameworkRoot }))
  }
  ...
  if (!serverResult.success) {
    const msgs = serverResult.logs.map((l) => l.message ?? String(l)).join("\n")
    throw new Error("patties build: server bundle failed\n" + msgs)
  }

  const serverOutput = serverResult.outputs.find((o) => o.kind === "entry-point") ?? serverResult.outputs[0]
  const serverEntryOut = serverOutput ? serverOutput.path : serverOutDir + "/server-entry.js"
  ...
  return { clientManifest: manifest, serverEntry: emitted.serverEntry, assets: emitted.assets }
}
...
function toPublicUrl(absPath: string, clientOutDir: string): string {
  const rel = absPath.startsWith(clientOutDir) ? absPath.slice(clientOutDir.length) : "/" + (absPath.split("/").pop() ?? "")
  return PUBLIC_PREFIX + rel.replace(/^\/+/, "")
}

function absolutize(p: string): string {
  if (p.startsWith("/")) return p.replace(/\/+$/, "")
  return process.cwd().replace(/\/+$/, "") + "/" + p.replace(/^\.\//, "").replace(/\/+$/, "")
}
