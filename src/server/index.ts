...
function buildRoutes(opts: ServerOptions): BunRoutes {
  ...
  if (opts.dev) {
    routes["/__patties_open"] = ((req: Request) => handleOpenInEditor(req)) as unknown as typeof routes[string]
    if (opts.devServer) {
      ...
      routes["/__patties_hmr"] = ((req: Request, server: unknown) => {
        ...
        if (s && typeof s.upgrade === "function" && s.upgrade(req)) return undefined
        return dev.fetch(req)
      }) as unknown as typeof routes[string]
    } else {
      routes["/__patties_hmr"] = (() =>
        new Response("HMR upgrade requires a real Bun.serve instance", { status: 426 })) as unknown as typeof routes[string]
    }
  }
  return routes
}
...
export function startServer(opts: ServerOptions) {
  ...
  const server = Bun.serve(serverOptions as unknown as Parameters<typeof Bun.serve>[0])
  if (opts.devServer) {
    opts.devServer.attachServer(server as unknown as { publish: (t: string, d: string) => number })
  }
  const bound = opts.unix ? `unix://${opts.unix}` : `http://${server.hostname}:${server.port}`
