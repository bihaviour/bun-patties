...
import type { RouteEntry, PattiesContext } from "../types.ts"
import { Shell } from "./shell.tsx"
import { renderMetaTags, defaultHead, type MetaInput } from "./meta.tsx"
...
const HMR_TAG_BYTES = new TextEncoder().encode(`<script>${HMR_CLIENT_SCRIPT}</script>`)
...
export function createRenderer(options: RenderOptions = {}): Renderer {
  ...
  return {
    async renderPage(entry, req, ctx) {
      try {
        const mod = (modules?.[entry.filePath] ?? (await import(entry.filePath))) as {
          default?: unknown
          meta?: MetaInput
        ...
        if (typeof Page !== "function") {
          throw new Error(`Page module ${entry.filePath} has no default-exported component`)
        }
        ...
        if (mod.head) {
          headNodes.push(<React.Fragment key="user-head">{mod.head()}</React.Fragment>)
        }

        const clientScriptUrl =
          manifest?.byEntry?.[entry.bunPattern] ??
...
function composeStream(reactStream: ReadableStream<Uint8Array>, dev: boolean): ReadableStream<Uint8Array> {
