...
import { runSubprocessBatch, type ModuleResult } from "./subprocess-runner.ts"
import type { RouteEntry } from "../types.ts"

export interface CollectedData {
  routes: RouteEntry[]
  islands: Array<{ name: string; relPath: string }>
  agents: Array<{ name: string; filePath: string; ok: boolean; data?: unknown; error?: string }>
  tools: Array<{ name: string; filePath: string; ok: boolean; data?: unknown; error?: string }>
  jobs: Array<{ name: string; filePath: string; ok: boolean; data?: unknown; error?: string }>
  middlewareDocComment: string | null
  envVars: Array<{ name: string; required: boolean; description?: string }>
}

export async function collect(appDir: string, env: Array<{ name: string; required: boolean; description?: string }>): Promise<CollectedData> {
  const [routes, islands, agentMods, toolMods, jobMods, middlewareDocComment] = await Promise.all([
...
): { name: string; filePath: string; ok: boolean; data?: unknown; error?: string } {
  if (!r) return { ...mod, ok: false, error: "no subprocess result" }
  return { ...mod, ok: r.ok, data: r.data, error: r.error }
}

async function extractMiddlewareDocComment(appDir: string): Promise<string | null> {
