...
export async function runSubprocessBatch(
  ...
  const sink = proc.stdin as { write: (chunk: string) => void; end: () => void } | null
  for (let i = 0; i < queries.length; i++) {
    const id = String(i)
    const line = JSON.stringify({ id, file: queries[i]!.file, kind: queries[i]!.kind }) + "\n"
    sink?.write(line)
  }
  sink?.end()
  ...
  const readAll = (async () => {
    while (true) {
      ...
      for (const line of lines) {
        const t = line.trim()
        if (!t) continue
        const parsed = JSON.parse(t) as { id: string; ok: boolean; data?: unknown; error?: string }
        const q = queries[Number(parsed.id)]!
        results.set(parsed.id, { query: q, ok: parsed.ok, data: parsed.data, error: parsed.error })
      }
    }
  })()
