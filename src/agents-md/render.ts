...
export function render(data: CollectedData): string {
  const out: string[] = []
  out.push("# AGENTS.md", "")
  out.push("> Generated by Patties. Do not edit by hand.", "")
  ...
  if (data.routes.length === 0) {
    ...
    for (const r of rows) out.push(`| ${r.file} | ${r.pattern} | ${r.methods} |`)
    out.push("")
  }
  ...
  if (data.agents.length === 0) {
    ...
    for (const a of rows) {
      if (!a.ok) {
        out.push(`| ${a.name} | _error_ | _error_ | _error: ${escapeCell(a.error ?? "unknown")}_ |`)
        continue
      }
      const d = a.data as { model?: string; tools?: string[]; triggers?: string[] }
      out.push(
  ...
  if (data.jobs.length === 0) {
    ...
    for (const j of rows) {
      if (!j.ok) {
        out.push(`| ${j.name} | _error_ | _error: ${escapeCell(j.error ?? "unknown")}_ |`)
        continue
      }
      const d = j.data as { schedule?: string; tz?: string }
      out.push(`| ${j.name} | ${escapeCell(d.schedule ?? "")} | ${escapeCell(d.tz ?? "")} |`)
    }
    out.push("")
  }
  ...
  if (data.envVars.length === 0) {
    ...
    for (const e of rows) {
      out.push(`| ${e.name} | ${e.required ? "yes" : "no"} | ${escapeCell(e.description ?? "")} |`)
    }
    out.push("")
  }
